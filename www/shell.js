...
(function () {
    ...
    /* ---------- dirty state indicator ----------
       Poll the active tab's dirty state and toggle .dirty class
       on the header save button. Green = clean, Red = dirty. */
    const updateSaveBtnDirtyState = () => {
        if (!saveBtn || current === 'status') return;
        if (tabIsDirty(current)) saveBtn.classList.add('dirty');
