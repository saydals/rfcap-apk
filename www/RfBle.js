...
const logHead = "[RfBLE]";
const plugin = Capacitor?.Plugins?.RfBle;
...
class RfBle extends EventTarget {
    constructor() {
        ...
        if (!plugin) {
            console.warn(`${logHead} Native RfBle plugin is not available`);
            return;
        }
    ...
    async getBondedDevices() {
        ...
        try {
            ...
            console.error(`${logHead} Failed to get bonded devices`, error);
            return [];
        }
    }
    ...
    async connect(path, options) {
        ...
        if (path && path.startsWith("spp:")) {
            const address = path.substring(4);
            return await this.connectSPP(address);
        }
    ...
    async connectSPP(address) {
        if (!plugin) return false;
        try {
            const result = await plugin.sppConnect({ address });
            const success = !!result?.success;
            if (success) {
                this.connected = true;
            ...
            console.error(`${logHead} SPP connect failed`, error);
            return false;
        }
    }
    ...
    async disconnectSPP() {
        if (!plugin) return false;
        try {
            const result = await plugin.sppDisconnect();
            const success = !!result?.success;
            this.connected = false;
            this.connectionId = null;
            this.dispatchEvent(new CustomEvent("disconnect", { detail: success }));
            ...
            console.error(`${logHead} SPP disconnect failed`, error);
            this.connected = false;
            this.connectionId = null;
            this.dispatchEvent(new CustomEvent("disconnect", { detail: false }));
            return false;
    ...
    async sendSPP(data) {
        if (!plugin || !this.connected) return { bytesSent: 0 };
        const bytes = new Uint8Array(data);
        const payload = uint8ArrayToBase64(bytes);
        try {
            const result = await plugin.sppWrite({ data: payload });
            const bytesSent = result?.bytesSent ?? bytes.byteLength;
            this.bytesSent += bytesSent;
            return { bytesSent };
        } catch (error) {
            console.error(`${logHead} SPP send failed`, error);
            return { bytesSent: 0 };
        }
    }
}

export default RfBle;
