...
(function () {
    ...
    function onStateChange() {
        ...
        if (!st.on) {
            ...
            lastAutoOn = false;
             ...
             /* Reset numpad position to default on disconnect */
             resetNumpadPos();
             return;
        }
    ...
    function installKeypad() {
        ...
        /* ---- pad placement & dragging ----
         ...
         function resetNumpadPos() {
             pos = null;
             try { localStorage.removeItem('rf-numpad-pos'); } catch (e) {}
             pad.style.left = '';
             pad.style.bottom = '';
             pad.style.transform = '';
             pad.style.transition = '';
         }
         window.resetNumpadPos = resetNumpadPos;
         function clampPos() {
        ...
        function commitValue(el) {
            ...
            const mn = el.getAttribute('min'), mx = el.getAttribute('max');
        ...
        function show(el) {
            /* Cell-to-cell move: commit the previous cell FIRST (same
               clamp + input/change as DONE). Without this the old cell kept
               its typed text on screen but never fired change, so Max/dirty
               stayed stale - the "outside tap loses input" symptom. */
            if (target && target !== el) {
                const prev = target;
                if (hideEl === prev) { try { clearTimeout(hideTO); } catch (e) {} hideTO = null; hideEl = null; }
                try { commitValue(prev); } catch (e) {}
                try { prev.blur(); } catch (e) {}
                try { prev.classList.remove('rf-editing'); } catch (e) {}
                if (editEl === prev) editEl = null;
            }
        ...
        css.textContent = [
            ...
            /* dirty cell highlight - light red tint, same visual weight as .rf-editing blue.
               Declared BEFORE .rf-editing so that .rf-editing (later in cascade) takes priority. */
